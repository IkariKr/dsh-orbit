...
### 0.4.1-rc.1 candidate - 2026-09-13

### Changed

- **Shipping baseline**: this candidate selects DeepSeek Harness `0.1.5-rc.2` as the shipping baseline. The post-freeze E9 release compatibility qualification passed and publishes it as `SUPPORTED`; `0.1.1-rc.2` remains a retained LEGACY profile — compatibility retained, capability eligible, not covered by the current release guarantee. Compatibility Refresh Patch Release per `docs/dsh-version-policy.md`.
- **E9 qualification**: frozen executable `d359a90165ad7bfeeee76d0dd47c2e5212ae6414` passed independent Final Review, formal copied-data candidate qualification, and verify; the evidence and scope boundary are recorded in `docs/release-attestations/v0.4.1-e9-qualification.md`.
...
- **Canonical smokes**: settings, authorization (6/6 matrix for both generations), session resume (pre-upgrade session resolve + recorded selection re-select, fail-closed without a fallback), and remote.mux transport (open → ready) are generation-aware and fail closed on a missing or unreviewed generation declaration.
