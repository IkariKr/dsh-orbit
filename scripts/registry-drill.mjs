...
const DRILL_DSH_COMMIT = process.env.DSH_ORBIT_DRILL_DSH_COMMIT;
...
const nodeEnv = (dataHome, name = null) => ({
  ...
  DSH_ORBIT_NODE_DSH_PROFILE: `dsh-${DRILL_DSH_VERSION}`,
...
async function main() {
  ...
  for (const [name, value] of Object.entries({
    DSH_ORBIT_DRILL_ORBIT_VERSION: DRILL_ORBIT_VERSION,
    DSH_ORBIT_DRILL_DSH_VERSION: DRILL_DSH_VERSION,
    DSH_ORBIT_DRILL_DSH_COMMIT: DRILL_DSH_COMMIT,
    DSH_ORBIT_DRILL_DSH_CLI_SHA256: DRILL_DSH_CLI_SHA256,
  })) {
    if (typeof value !== "string" || value.length === 0) throw new Error(`${name} is required for candidate-bound mounted evidence`);
  }
  ...
  evidence.dsh = {
    version: DRILL_DSH_VERSION,
    commitSha: DRILL_DSH_COMMIT,
    cliSha256: DRILL_DSH_CLI_SHA256,
  };
  ...
  async function deployNode(name, dataHome, endpoint, port) {
    ...
    const report = createCompatibilityReport({
      ...
      candidate: { dshVersion: DRILL_DSH_VERSION, profile: `dsh-${DRILL_DSH_VERSION}` },
      checks,
