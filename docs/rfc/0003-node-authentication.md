...
2. **Per-node Hub-to-node trust continuation** — the Hub uses a per-node Hub service identity (never a reused operator account and never a fleet-wide credential). The v0.4 specialization is RFC-0008 + RFC-0010: the Hub authenticates to the node's Orbit route ingress with that per-node Ed25519 identity, and the node-local DSH compatibility adapter then satisfies whatever authenticated gateway/trusted-client contract the supported DSH version requires. The Hub never receives the node's internal DSH/gateway secret; any such secret remains node-local.
